  ...
  if (data.ns.coll !== "audits" && data.ns.coll !== "traffics" && data.ns.coll !== "sessions") {
    ...
    if (data2) {
      await traffic
        .findOneAndUpdate(
          { Day: formateddate },
          { $set: { Database_Changes: data2.Database_Changes + 1 } },
        )
        .exec();
    } else {
...
Database.on(
  ...
  console.error.bind(
    console,
    chalk.bgRedBright.bold(" [MongoDB] Failed To Connect To Database : \n"),
  ),
);
Database.once("open", () => {
  console.log(
    chalk.bgGreenBright.bold(" [MongoDB] Successfuly Connected To Database "),
  );
});
...
app.use(
  session({
    ...
    cookie: {
      maxAge: 8.64e7 * 7, // 7 days
    },
  }),
);
app.listen(`${PORT}`, () => {
  console.log(
    chalk.bgGreenBright.bold(" [Express] WebApp Successfully Booted "),
  );
});
